...
async def update_it(client, message):
    msg_ = await message.reply_text("`🔄️ Updating Please Wait!`")
    ...
    if "now" == text:
        ...
        await msg_.edit(f"**ℹ️ Update Available ℹ️**\n\nFrom [REPO]({REPO_})\nCHANGELOG:\n\n{clogs}\n\nDo `/{BotCommands.UpdateCommand} now` to Update BOT.", parse_mode="Markdown",disable_web_page_preview=True)
        return
