...
from bot import DRIVE_NAME, DRIVE_ID, INDEX_LINK, telegraph_token, INDEX_HOMEPAGE_URL, AUTHOR_NAME, AUTHOR_URL, OWNER_USERNAME, SEARCH_TITLE, ORDER_SORT, TELEGRAPH_INDEX, TELEGRAPH_DRIVE, TELEGRAPH_VIEW
...
class GoogleDriveHelper:
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            if nxt_page == 1 :
                content += f'<b><a href="https://telegra.ph/{self.path[nxt_page]}">➡️</a></b>'
                nxt_page += 1
            else :
                if prev_page <= self.num_of_path:
                    content += f'<b><a href="https://telegra.ph/{self.path[prev_page]}">⬅️</a></b>'
                    prev_page += 1
    ...
    def drive_list(self, fileName):
                ...
                for file in response:
                    ...
                    if x:
                        if add_title_msg == True:
                            msg = f'<h3>SEARCH RESULT FOR : {fileName}</h3><br><b><a href="{INDEX_HOMEPAGE_URL}"> Index Homepage </a></b> ||<b><a href="https://t.me/{OWNER_USERNAME}"> Owner </a></b><br><br>'
                            add_title_msg = False
                        ...
                        if file.get('mimeType') == "application/vnd.google-apps.folder":  # Detect Whether Current Entity is a Folder or File.
                            msg += f"🗃️<code>{file.get('name')}</code> <b>(folder)</b><br>" \
                                   f"<b><a href='https://drive.google.com/drive/folders/{file.get('id')}'>{TELEGRAPH_DRIVE}</a></b>"
                            ...
                            msg += f"📄<code>{file.get('name')}</code> <b>({self.get_readable_file_size(file.get('size'))})</b><br>" \
                                   f"<b><a href='https://drive.google.com/uc?id={file.get('id')}&export=download'>{TELEGRAPH_DRIVE}</a></b>"
                            if INDEX_LINK[INDEX] is not None:
                                url_path = "/".join([requests.utils.quote(n, safe ='') for n in self.get_recursive_list(file, parent_id)])
                                url = f'{INDEX_LINK[INDEX]}/{url_path}'
                    ...
                    if (content_count==TELEGRAPHLIMIT):
                        msg = f'<h3>Too many results for : {fileName}</h3><br>'
        ...
        msg = f" <b>Search Results for :</b> ➼ {fileName}  "
